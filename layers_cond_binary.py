...
# havent played with anything here
...
class TransformerPositionalEmbedding(nn.Module):
    ...
    def forward(self, timestep):
        #print("transformerPositionalEmbedding shape of timestep before  pe_matrix:", timestep)
        # [bs, d_model]
        self.pe_matrix = self.pe_matrix.to(timestep.device)
        #print("what is the pe_matrix inside transformer doing:", self.pe_matrix)
        #print("pe[timestep]:", self.pe_matrix[timestep].to(timestep.device))
...
class ConvDownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers, time_emb_channels, cond_emb_channels, num_groups, downsample=True):
    ...
    def forward(self, input_tensor, time_embedding, cond_embedding):
        x = input_tensor
        for resnet_block in self.resnet_blocks:
            x = resnet_block(x, time_embedding, cond_embedding)
        if self.downsample:
            x = self.downsample(x)
        return x
...
class ConvUpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers, time_emb_channels,cond_emb_channels, num_groups, upsample=True):
...
class AttentionDownBlock(nn.Module):
    ...
    def forward(self, input_tensor, time_embedding, cond_embedding):
        x = input_tensor
        # TODO: change RESNETBLOCK and SELFATTENTIONBLOCK
...
class ResNetBlock(nn.Module):
    """
    A ResNet block with FiLM conditioning.
    It applies two convolutional blocks (ConvBlock), then modulates the features using
    scale (γ) and shift (β) parameters computed from the provided time and conditional embeddings.
    If no embeddings are provided, it behaves like a standard ResNet block.
    """
    def __init__(self, in_channels, out_channels, *, time_emb_channels=None, cond_emb_channels=None, num_groups=8):
        super(ResNetBlock, self).__init__()
        # First convolutional block: processes input and outputs features with out_channels.
        self.block1 = ConvBlock(in_channels, out_channels, groups=num_groups)
        # Second convolutional block: further processes the features.
        self.block2 = ConvBlock(out_channels, out_channels, groups=num_groups)
        
        # Residual connection: if in_channels doesn't match out_channels, use a 1x1 convolution to align dimensions.
        self.residual_conv = nn.Conv2d(in_channels, out_channels, 1) if in_channels != out_channels else nn.Identity()

        # Determine the total embedding dimension from time and conditional embeddings.
        combined_emb_channels = 0
        if time_emb_channels is not None:
            combined_emb_channels += time_emb_channels
        if cond_emb_channels is not None:
            combined_emb_channels += cond_emb_channels

        # If any conditioning is available, create an MLP to output FiLM parameters.
        if combined_emb_channels > 0:
            # The FiLM MLP outputs 2 * out_channels (one set for scale and one for shift).
            self.film_proj = nn.Sequential(
                nn.SiLU(),
                nn.Linear(combined_emb_channels, out_channels * 2)
            )
        else:
            self.film_proj = None
    ...
    def forward(self, x, time_embedding=None, cond_embedding=None):
        ...
        if self.film_proj is not None:
            embeddings = []
            # add time_emb to the list.
            if time_embedding is not None:
                embeddings.append(time_embedding)
            # add the conditional embedding
            if cond_embedding is not None:
                embeddings.append(cond_embedding)
            # Concatenate embeddings along the last dimension.
            # Combined shape becomes [B, (time_emb_channels + cond_emb_channels)]
            combined_emb = torch.cat(embeddings, dim=-1)
            # Pass the combined embedding through the FiLM projection MLP.
            # Output shape: [B, out_channels * 2]
            film_params = self.film_proj(combined_emb)
            # Split the output into scale (gamma) and shift (beta) parameters.
            # Each has shape [B, out_channels]
            gamma, beta = film_params.chunk(2, dim=-1)
            # Reshape gamma and beta to allow broadcasting across spatial dimensions.
            # New shape: [B, out_channels, 1, 1]
            gamma = gamma[:, :, None, None]
            beta = beta[:, :, None, None]
            # Apply FiLM conditioning: multiply h element-wise by gamma and add beta.
            h = gamma * h + beta
        
        # Pass the modulated features through the second convolutional block.
        h = self.block2(h)
        # Compute the residual path: if needed, adjust the input using a 1x1 convolution.
        # Then add it to the output of block2.
...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, *, time_emb_channels=None, cond_emb_channels=None, num_groups=8):
        ...
        self.conditional_embedding_projectile = (
            nn.Sequential(nn.SiLU(), nn.Linear(cond_emb_channels, out_channels))
            if cond_emb_channels 
            else None   
        )
    ...
    def forward(self, x, time_embedding=None, cond_embedding=None):
        ...
        cond_emb = self.conditional_embedding_projectile(cond_embedding)
        cond_emb = cond_emb[:, :, None, None]
        
        #print(f"Inside Resnet: shape of cond emb: {cond_emb.shape}")
        #print(f"Inside Resnet: shape of time emb: {time_emb.shape}")
        #print(f"Inside Resnet: shape of h: {h.shape}")


        '''
        Inside Resnet, shape of h: torch.Size([32, 256, 16, 16])
        Inside Resnet, shape of time embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of h: torch.Size([32, 256, 32, 32])
        Inside Resnet, shape of time embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of h: torch.Size([32, 256, 32, 32])
        Inside Resnet, shape of time embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 256, 1, 1])
        Inside Resnet, shape of h: torch.Size([32, 128, 64, 64])
        Inside Resnet, shape of time embedding: torch.Size([32, 128, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 128, 1, 1])
        Inside Resnet, shape of h: torch.Size([32, 128, 64, 64])
        Inside Resnet, shape of time embedding: torch.Size([32, 128, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 128, 1, 1])
        Inside Resnet, shape of h: torch.Size([32, 128, 128, 128])
        Inside Resnet, shape of time embedding: torch.Size([32, 128, 1, 1])
        Inside Resnet, shape of cond embedding: torch.Size([32, 128, 1, 1])
        '''

        x = time_emb + cond_emb + h
